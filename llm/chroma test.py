...
# class snowflakeEmbedding(chromadb.EmbeddingFunction):
#     def __call__(self, input: chromadb.Documents) -> chromadb.Embeddings:
...
while True:
    ...
    # now = datetime.now()
    # day = now.strftime("%A")        # Day name (e.g., Monday)
    # date = now.strftime("%Y-%m-%d") # Date (YYYY-MM-DD)
    # exactTime = now.strftime("%H:%M") # Time (HH:MM:SS)
    # # print(day)
    # # print(date)
    # # print(exactTime)
    # konteks += f"\nRight now, its is {day}, the date is {date},and the time is {"17:11"}"
    print(f"konteks: {konteks}\n")

    response = chat(
        'llama3.2', #best overall, sekali ngasal pas ditanya kantor buka, jadi mending gk usah kasih botnya hari dan waktu
        # 'command-r7b:latest', #tau2 gjls ngomong cina ?
        # 'phi4-mini:latest', #di not pass the information consistently
        # 'granite4:3b',    #agak miss grammar b indonya
        # 'ministral-3:3b', #in markdown format | overall good- but fail the is it open now test
        # "gemma3",#Serting reply in english
        messages =
        [
            {
                'role': 'system',
                'content': 
                    f"You are representing IDE Asia, you will have a conversation with a customer, only answer with according to the following Information : {konteks}. Say you dont know if no relevant information is provided"
            },
    ...
    print(f"response took {time.perf_counter() - start} detik")
